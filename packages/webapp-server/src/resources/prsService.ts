...
import type { PrsService, StartReviewResult } from "reviewflow-modules";
...
const startReviewForUser = async (
  mongoStores: MongoStores,
  prId: string,
  loggedInUser: AuthenticatedWsUser,
  ...
  // would put it, so there is nothing to start. A press that follows one this
  // document already records lands here too
  const isAskedAgain =
    (pr.reviews?.reviewed?.some(({ id }) => id === user.id) ?? false) &&
    (pr.reviews?.reviewRequested?.some(({ id }) => id === user.id) ?? false);
...
export const createPrsService = ({
  ...
  /**
   * Starting a review comments on the pull request, and the document that
   * decides whether it still has to be started is written back only once the
   * two github calls have returned. A second send arriving inside that window
   * — the reviewer's websocket reconnected, the link is open twice — reads the
   * same pre-write document and comments again, so it is answered with the
   * send already in flight instead of being run.
   *
   * In process on purpose: the webapp server runs as one node process, which
   * its liwi subscriptions already require.
   */
  const inFlightStartReviews = new Map<string, Promise<StartReviewResult>>();
  ...
  return {
    ...
    operations: {
      startReview: async ({ prId }, loggedInUser) => {
        ...
        const key = `${prId}:${user.id}`;

        const inFlight = inFlightStartReviews.get(key);
        if (inFlight) return inFlight;

        const started = startReviewForUser(mongoStores, prId, user).finally(
          () => {
            inFlightStartReviews.delete(key);
          },
        );
        inFlightStartReviews.set(key, started);
        return started;
      },
    },
  };
