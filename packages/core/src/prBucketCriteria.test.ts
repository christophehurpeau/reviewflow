...
const orgWithTeams = { orgId: 1, teams: [{ id: 9, name: "dev", slug: "dev" }] };
const orgWithoutTeams = { orgId: 1, teams: [] };
const otherOrg = { orgId: 2, teams: [{ id: 8, name: "ops", slug: "ops" }] };

const withTeams = { userId: 42, orgs: [orgWithTeams] };
const withoutTeams = { userId: 42, orgs: [orgWithoutTeams] };
const acrossOrgs = { userId: 42, orgs: [orgWithTeams, otherOrg] };
const withoutOrg = { userId: 42, orgs: [] };
...
describe("buildPrBucketQuery", () => {
  ...
  it("matches nothing when the user belongs to no org", () => {
    for (const bucket of buckets) {
      expect(criteriaOf(bucket, withoutOrg)["account.id"]).toEqual({ $in: [] });
    }
  });
